...
export default {
  ...
  methods: {
    ...
    loadAccounts() {
      ...
      ifetch.post("api/accounts", {}).then(data => {
        if (data.code != 0) {
    ...
    showDialogNewAccount() {
      this.showDialog({
        ...
        mainClick: (data) => {
          ...
          ifetch.post("api/accounts/add", request).then(resp => {
            if (resp.code != 0) {
              this.showErrorDialog(resp.msg);
              return
            }
            ...
            this.accounts.push({ username: data.username, owner: data.owner });
    ...
    showDialogChangePassword(account) {
      this.showDialog({
        ...
        mainClick: (data) => {
          ...
          ifetch.post("api/accounts/changePwd", request).then(resp => {
            if (resp.code != 0) {
              this.showErrorDialog(resp.msg);
              return
            }
    ...
    showDialogDeleteAccount(account, idx) {
      this.showDialog({
        ...
        mainClick: () => {
          ...
          ifetch.post("api/accounts/delete", {
            "username": account.username,
          }).then(resp => {
            if (resp.code != 0) {
              this.showErrorDialog(resp.msg);
