...
export default {
  ...
  methods: {
    ...
    loadData(saveState, fetchTags) {
      ...
      if (fetchTags) {
        ifetch.post("/api/tags", {}).then(data => {
          if (data.code != 0) {
            that.showErrorDialog(data.msg);
            return
          }
          var json = data.data
          that.tags = json;
    ...
    showDialogAdd() {
      this.showDialog({
        ...
        mainClick: (data) => {
          ...
          ifetch.post("api/bookmarks/add", data).then(data => {
            if (data.code != 0) {
              this.showErrorDialog(data.msg);
              return
            }
    ...
    showDialogEdit(item) {
      ...
      var strTags = book.tags_detail.map(tag => tag.name).join(", ");
      ...
      this.showDialog({
        ...
        fields: [{
          ...
          value: strTags,
          separator: ",",
          dictionary: book.tags_detail.map(tag => tag.name)
        }
          // ,
          // {
          // name: "makePublic",
    ...
    showDialogDelete(items) {
      ...
      this.showDialog({
        ...
        mainClick: () => {
          ...
          // 删除书签
          ifetch.post("api/bookmarks/delete", {
            id: ids[0]
          }).then(data => {
            if (data.code != 0) {
              this.showErrorDialog(data.msg);
