...
<div class="bookmark" :class="{list: listMode, 'no-thumbnail': hideThumbnail, selected: selected}">
	...
	<a class="bookmark-link" :href="mainURL" target="_blank" rel="noopener">
		...
		<p class="excerpt" v-if="excerptVisible">{{excerpt}}写</p>
...
export default {
  ...
  computed: {
    ...
    thumbnailStyleURL() {
      // http://127.0.0.1:38112/api/bookmarks/showShot?image_url=1691566797806980000.jpg
      return {
        backgroundImage: `url("/api/bookmarks/showShot?image_url=${this.imageURL}")`
      }
    },
    eventItem() {
      return {
